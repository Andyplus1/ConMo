...
from utilities.guidance_utils_multimasks_interfere_reposition import (
    get_timesteps,
    register_time,
    register_guidance,
    calculate_losses,
...
class Guidance(nn.Module):
    ...
    def get_data(self):
        ...
        masks_path = self.config["mask_path"]
        video_masks = []
        for mask_path in masks_path:
            mask = list(Path(mask_path).glob("*.png")) + list(Path(mask_path).glob("*.jpg"))
            mask = sorted(mask, key=lambda x: int(x.stem))
        
            video_mask = torch.stack([ToTensor()(Image.open(img)) for img in mask]).cuda()
            video_mask = video_mask[: self.config["max_frames"]]
            video_mask = video_mask>0.5
            if video_mask.shape[1] == 4:
    ...
    def guidance_step(self, x, i, t):
        ...
        for _ in tqdm(range(self.optimization_steps)):
            ...
            for (orig_name, orig_features), (target_name, target_features) in zip(
                ...
                losses = calculate_losses(orig_features.detach(), target_features,self.video_masks, self.config)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_path", type=str, default="/root/autodl-tmp/rebuttal/DMT-to-MTASAP/configs/guidance_config_up_withmultimasks_interfere_resize&reposition.yaml")
