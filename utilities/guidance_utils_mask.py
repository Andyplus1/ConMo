...
def mask_or(mask_anchor,mask_all):
    ...
    return mask_all | mask_anchor.unsqueeze(0)
...
def mask_or_bbox(mask_anchor,mask_all):
    ...
    return mask_all_trangle | mask_anchor_trangle.unsqueeze(0)
...
def expand_true_regions(matrix,h_raitio,w_ratio): 
    ...
    for i in range(matrix.shape[0]):
        ...
        expanded_matrix[i,0,y1:y2+1,x1:x2+1] = True
...
# def expand_true_regions(matrix, h_ratio: float, w_ratio: float) -> torch.Tensor: 
#     if not isinstance(matrix, torch.Tensor) or matrix.dim() != 4:
#         raise ValueError("输入必须是形状为 (n, c, h, w) 的四维 PyTorch Tensor")
...
#     # 找到所有为 True 的索引
#     true_mask = matrix[:, 0]  # 选择第一通道并生成掩码
#     true_indices = torch.nonzero(true_mask, as_tuple=False)

#     if true_indices.numel() > 0:
#         # 获取每个 True 区域的最小和最大坐标
#         h_indices = true_indices[:, 1]
#         w_indices = true_indices[:, 2]
...
#         (x1, y1), (x2, y2) = scale_bbox((w_min.item(), h_min.item()), (w_max.item(), h_max.item()), w_ratio, h_ratio)
...
#         # 扩展区域设为 True
#         expanded_matrix[:, 0, y1:y2 + 1, x1:x2 + 1] = True
...
def calculate_losses(orig_features, target_features, masks,config):
    ...
    if config["features_diff_loss_weight"] > 0: #True
        ...
        compressed_tensor = F.interpolate(masks, size=orig.shape[-2:], mode='bilinear')
        ...
        for i in range(len(orig)):
            mask_anchor1 = mask_bool[i]
            mask_anchor1_4gen = mask_bool_4gen[i]
            # print("test bbox mask"+str(config["bbox_mask"]))
            if config["bbox_mask"]:
                mask_target = mask_or_bbox(mask_anchor1,mask_bool)
                mask_target_4gen = mask_or_bbox(mask_anchor1_4gen,mask_bool_4gen)
            else:
                mask_target = mask_or(mask_anchor1,mask_bool)
                mask_target_4gen = mask_or(mask_anchor1_4gen,mask_bool_4gen)
            # mask_target = mask_or(mask_anchor1,mask_bool)
            orig_anchor1 = orig[i].squeeze(0).repeat(24,1,1,1)
            target_anchor1 = target[i].squeeze(0).repeat(24,1,1,1)
            orig_masked = new_mean(orig, mask_target)
            orig_anchor1_masked = new_mean(orig_anchor1, mask_target)
            orig_diffs_masked = orig_masked - orig_anchor1_masked
            target_masked = new_mean(target, mask_target_4gen)
            target_anchor1_masked = new_mean(target_anchor1, mask_target_4gen)
            ...
            orig_unmasked = new_mean(orig, ~mask_target)
            orig_anchor1_unmasked = new_mean(orig_anchor1, ~mask_target)
            orig_diffs_unmasked = orig_unmasked - orig_anchor1_unmasked
            target_unmasked = new_mean(target, ~mask_target_4gen)
            target_anchor1_unmasked = new_mean(target_anchor1, ~mask_target_4gen)
