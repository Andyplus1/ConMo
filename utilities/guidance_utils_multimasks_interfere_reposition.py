...
def reposition_true_regions_mask(matrix,h_ratio,w_ratio): 
    """
    根据给定的比例移动四维张量中的真实区域掩码。
    
    参数：
    - matrix: 布尔掩码，形状为 (bs, n, h, w) 的四维 torch.Tensor。
    - h_ratio: 垂直方向移动的比例，例如 0.2 表示移动 20%。
    - w_ratio: 水平方向移动的比例，例如 0.2 表示移动 20%。
    
    返回：
    - 移动后的布尔掩码。
    """
    bs, n, h, w = matrix.shape
    # 计算水平方向和垂直方向实际移动的步数
    
    
    # 创建一个存放结果的张量
    rolled_matrix = torch.zeros_like(matrix, dtype=torch.bool)
        
    # 对每个批次和每个通道独立处理
    for b in range(bs):
        horizontal_shift = int(w * w_ratio[b])
        vertical_shift = int(h * h_ratio[b])
        for i in range(n):
            # 使用 torch.roll 移动掩码
            rolled_matrix[b, i] = torch.roll(matrix[b, i], shifts=(vertical_shift, horizontal_shift), dims=(0, 1))
    
    return rolled_matrix

#这段代码也有一点问题,11.6更新，以最低点中心为参考点
def rescale_tensor(tensor, h_rescale, w_rescale): 
    ...
    # 计算中心
    y_up = true_indices[:, 0].float().min().item()
    y_down = true_indices[:, 0].float().max().item()
    x_left = true_indices[:, 1].float().min().item()
    x_right = true_indices[:, 1].float().max().item()
    
    bottom_point = (y_down,(x_right+x_left)/2)
    subject_width = x_right - x_left + 1 
    subject_height = y_down - y_up + 1

    new_subject_width = subject_width*w_rescale
    new_subject_height = subject_height*h_rescale
    
    new_x_left = int(bottom_point[1]-new_subject_width/2)+1
    new_x_right = int(bottom_point[1]+new_subject_width/2)+1
    new_y_up = int(y_down-new_subject_height)+1
    new_y_down = int(y_down + 1)

    original_height, original_width = tensor.size()
    output_tensor = torch.zeros((original_height, original_width), dtype=torch.bool)
    output_tensor[new_y_up:new_y_down,new_x_left:new_x_right] = True
...
def calculate_losses(orig_features, target_features, masks,config):
    ...
    if config["features_diff_loss_weight"] > 0: #True
        ...
        if config.reposition:
            mask_bool_4gen = reposition_true_regions_mask(mask_bool,config.h_ratio,config.w_ratio)
