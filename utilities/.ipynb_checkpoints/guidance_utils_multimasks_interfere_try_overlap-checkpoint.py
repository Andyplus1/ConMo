...
def calculate_losses(orig_features, target_features, masks,config):
    ...
    for j in range(mask_bool.shape[0]):
        non_full_false_indices = torch.any(mask_bool[j], dim=(1, 2))  
        valid_indices = torch.nonzero(non_full_false_indices).squeeze()
    ...
    for i in range(mask_bool.shape[1]):
        ...
        for j in range(mask_bool.shape[0]):
            ...
            elif config["interfere_mask"]: # False
        ...
        for j in range(mask_bool.shape[0]):
            ...
            if i not in  valid_indices_list[j]:
                continue
            ...
            if config["bbox_mask"]:
                ...
                mask_target = mask_or(mask_anchor1,mask_bool[j][valid_indices_list[j]])
                mask_target_4gen = mask_or(mask_anchor1_4gen,mask_bool_4gen[j][valid_indices_list[j]])
                # mask_target_all = mask_target_all | mask_target
                # mask_target_all_4gen = mask_target_all_4gen | mask_target_4gen
             
            orig_anchor1 = orig[i].squeeze(0).repeat(len(valid_indices_list[j]),1,1,1)
            target_anchor1 = target[i].squeeze(0).repeat(len(valid_indices_list[j]),1,1,1)
            orig_masked = new_mean(orig[valid_indices_list[j]], mask_target.unsqueeze(1))
            orig_anchor1_masked = new_mean(orig_anchor1, mask_target.unsqueeze(1))

            # from IPython import embed;embed()
            sum_weights = sum(config["cm_weight"])
            if sum_weights>0:
            ...
            target_masked = new_mean(target[valid_indices_list[j]], mask_target_4gen.unsqueeze(1))
    ...
    for valid_indices in valid_indices_list:
        sum_pages+=len(valid_indices)
    # print("test2")
    # from IPython import embed;embed()
    features_diff_loss /= sum_pages
