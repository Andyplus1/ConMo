...
def calculate_losses_new(orig_features, background_features,target_features, config):
    orig = orig_features
    ...
    if config["features_diff_loss_weight"] > 0: #True
        ...
        orig1 = orig1.mean(dim=(2, 3), keepdim=True)  # t d 1 1
        target1 = target1.mean(dim=(2, 3), keepdim=True)
        
        orig2 = orig2.mean(dim=(2, 3), keepdim=True)  # t d 1 1
        target2 = target2.mean(dim=(2, 3), keepdim=True)
        
        for i in range(len(orig)):
            orig_anchor1 = orig1[i]
            target_anchor1 = target1[i]
            orig_diffs1 = orig1 - orig_anchor1  # t d 1 1
            target_diffs1 = target1 - target_anchor1  # t d 1 1
            # features_diff_loss += 1 - F.cosine_similarity(target_diffs1, orig_diffs1.detach(), dim=1).mean()
            features_diff_loss += 1 - F.cosine_similarity(target_diffs1, orig_diffs1, dim=1).mean()
            
            orig_anchor2 = orig2[i]
            target_anchor2 = target2[i]
            orig_diffs2 = orig2 - orig_anchor2  # t d 1 1
            target_diffs2 = target2 - target_anchor2  # t d 1 1
            # features_diff_loss += 1 - F.cosine_similarity(target_diffs2, orig_diffs2.detach(), dim=1).mean()
            features_diff_loss += 1 - F.cosine_similarity(target_diffs2, orig_diffs2, dim=1).mean()
            from IPython import embed;embed()
